...
final class PhaseFinStudentFeeReopenHttpApiPostgreSqlTest extends PostgreSqlIntegrationTestCase
{
    ...
    {
        $schoolId = $this->createSchool('SCH-SFR-'.$suffix, 'StudentFeeReopen '.$suffix);
        $yearId = $this->createAcademicYear('AY-SFR-'.$suffix);
    ...
    #[Test]
    public function manager_can_reopen_cancelled_student_fee(): void
    {
        $ctx = $this->seedContext('R1');
        ...
        ], ['X-Idempotency-Key' => 'fin-sf-r-create'])->assertCreated()->json('data.student_fee_id');

        $this->postJson('/api/v1/finance/student-fees/'.$studentFeeId.'/cancel', [], [
            'X-Idempotency-Key' => 'fin-sf-r-off',
        ])->assertOk();

        $this->postJson('/api/v1/finance/student-fees/'.$studentFeeId.'/reopen', [], [
            'X-Idempotency-Key' => 'fin-sf-r-on',
        ])
            ->assertOk()
