...
use App\Application\Finance\Results\VoidPaymentResult;
use App\Domain\Finance\Events\PaymentVoided;
...
final class VoidPaymentHandler implements CommandHandler
{
    private const COMMAND_NAME = 'VoidPayment';
    ...
    public function handle(Command $command): VoidPaymentResult
    {
        assert($command instanceof VoidPaymentCommand);
        $key = FinanceIdempotencyGuard::requireKey($command->idempotencyKey);
        $cached = $this->idempotency->find($key, self::COMMAND_NAME);
        if ($cached !== null) {
            return VoidPaymentResult::fromIdempotency(
        ...
        if ($payment === null) {
            return VoidPaymentResult::failure(['finance.payment_not_found']);
        }
        if (! PaymentStatus::isPosted($payment->status)) {
            return VoidPaymentResult::failure(['finance.payment_not_posted']);
        }

        $fee = $this->studentFees->findById($command->schoolId, $payment->studentFeeId);
        if ($fee === null) {
            return VoidPaymentResult::failure(['finance.student_fee_not_found']);
        }

        $studentId = $this->studentFees->findStudentIdByEnrollment($command->schoolId, $fee->enrollmentId);
        if ($studentId === null) {
            return VoidPaymentResult::failure(['finance.enrollment_not_found']);
        }
        ...
        $newStatus = $this->unitOfWork->transaction(function () use ($command, $key, $payment, $fee, $studentId, $notes, $at): ?int {
            $voided = $this->payments->voidPayment(
                $command->schoolId,
                $command->paymentId,
                $at,
                $command->voidedBy,
            ...
            $this->ledger->appendPaymentRefunded(
                $command->schoolId,
                $studentId,
                $fee->academicYearId,
                $payment->amount,
            ...
            $this->outbox->stage(new PaymentVoided(
                $command->paymentId,
                $command->schoolId,
        ...
        if ($newStatus === null) {
            return VoidPaymentResult::failure(['finance.payment_void_failed']);
        }

        return VoidPaymentResult::success($command->paymentId, $newStatus);
    }
}
