...
use App\Application\Results\Results\FinalizeGpaResult;
use App\Domain\Results\Data\PersistOperationalGpaResultData;
use App\Domain\Results\Events\GpaFinalized;
...
final class FinalizeGpaHandler implements CommandHandler
{
    private const COMMAND_NAME = 'FinalizeGpa';
    ...
    public function handle(Command $command): FinalizeGpaResult
    {
        assert($command instanceof FinalizeGpaCommand);
        ...
        if ($cached !== null) {
            return FinalizeGpaResult::fromIdempotency(
        ...
        $fingerprint = hash('sha256', $annual->annualResultId.'|'.$annual->sourceFingerprint.'|gpa-official');
        ...
        $gpaResultId = $this->unitOfWork->transaction(function () use (
            ...
            $id = $this->gpaResults->insertOfficial(new PersistOperationalGpaResultData(
            ...
            $this->outbox->stage(new GpaFinalized(
                gpaResultId: $id,
                schoolId: $command->schoolId,
                enrollmentId: $command->enrollmentId,
                studentId: $enrollment['student_id'],
        ...
        return FinalizeGpaResult::success(
            $gpaResultId,
            $version,
